...
/// <param name="FirstStripeCustomerId">First customer in the client's Stripe cluster, or null → the app would create one.</param>
public readonly record struct ConvertDraftInput(
    AccountType AccountType,
    string CurrencyCode,
    bool HasPackage,
    string? EffectiveStripePriceId,
    string? FirstStripeCustomerId);
...
public static class ConvertDrafter
{
    public static ConvertDraft Draft(ConvertDraftInput s)
    {
        ...
        var wouldCreateCustomer = string.IsNullOrWhiteSpace(s.FirstStripeCustomerId);
        var customerId = wouldCreateCustomer ? null : s.FirstStripeCustomerId;
